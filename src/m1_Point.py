...
class Point(object):
    """Represents a point in two space"""
    ...
    def move_to(self, x, y):
        self.totaldistance = self.totaldistance + math.sqrt((self.x - x)**2
                                                            + (self.y - y)**2)
        self.x = x
        self.y = y
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.beginx)**2 + (self.y - self.beginy)**2)
    ...
    def halfway_to(self, p2):
        self.newx = (self.x + p2.x)/2
        self.newy = (self.y + p2.y)/2
        return Point(self.newx, self.newy)
